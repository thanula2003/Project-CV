...
// ── ATS-accurate SVG thumbnails (all single-column, no photos/sidebars) ──────

// Classic: centred serif name, thick bottom border, left-aligned sections, cream chips
...
// Modern: left-aligned bold name, blue bottom border, blue section headings, blue chips
...
// Minimal: light weight tall name, thin hairline rules, outline chips no fill
function MinimalThumb() {
  return (
    <svg viewBox="0 0 180 240" xmlns="http://www.w3.org/2000/svg" className="template-thumb">
      {/* very large light-weight name */}
...
// Executive: centred UPPERCASE name, gold double rules top/bottom header, gold section underlines
function ExecutiveThumb() {
  return (
    <svg viewBox="0 0 180 240" xmlns="http://www.w3.org/2000/svg" className="template-thumb">
      ...
      {/* gold double-rule bottom of header */}
    ...
    <div className="page-wrapper">
      <div className="page-card ts-card">
        ...
        <div className="ts-grid">
          {TEMPLATES.map((t) => {
            ...
            return (
              ...
              >
                {t.badge && <span className="ts-badge" style={isSelected ? { background: t.accent } : {}}>{t.badge}</span>}

                <div className={`ts-check ${isSelected ? "ts-check--visible" : ""}`}
        ...
        <div style={{
          ...
          <span>
            <strong style={{ color: "var(--text-h)" }}>ATS-safe guarantee:</strong> All templates use a single-column layout with no photos, tables for layout, or text boxes — so applicant tracking systems can parse your CV without errors.
          </span>
        </div>
        ...
        <div className="form-actions">
          ...
          <button
            ...
            style={{ minWidth: 180 }}
          >
            View CV with {TEMPLATES.find(t => t.id === selected)?.name} →
          </button>
        </div>
      </div>
    </div>
